...
<div class="max-w-2xl">
    ...
    <form method="POST" action="{{ isset($dokter->id) ? route('admin.dokter.update', $dokter) : route('admin.dokter.store') }}" enctype="multipart/form-data" class="bg-white rounded-xl shadow p-6 space-y-5">
        ...
        <div>
            ...
            <textarea name="bio" rows="3" class="w-full border border-gray-300 rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-green-500">{{ old('bio', $dokter->bio) }}</textarea>
        </div>
