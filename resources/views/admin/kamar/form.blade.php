...
<div class="max-w-2xl">
    ...
    <form method="POST" action="{{ isset($kamar->id) ? route('admin.kamar.update', $kamar) : route('admin.kamar.store') }}" enctype="multipart/form-data" class="bg-white rounded-xl shadow p-6 space-y-5">
        ...
        <div>
            ...
            <textarea name="deskripsi" rows="4" required class="w-full border border-gray-300 rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-green-500">{{ old('deskripsi', $kamar->deskripsi) }}</textarea>
        </div>
        <div>
