    ...
    <div class="container mx-auto px-4">
        <div class="flex items-center justify-between py-3">
            ...
            <a href="{{ route('home') }}" class="flex items-center gap-3">
                ...
                @endif
                <div>
                    <div class="font-playfair font-bold text-primary text-lg leading-tight">{{ $settings['nama_rs'] ?? 'RSUD Landak' }}</div>
                    <div class="text-xs text-gray-500">{{ $settings['tagline'] ?? 'Melayani dengan Sepenuh Hati' }}</div>
                </div>
            </a>
            ...
            <div class="hidden md:flex items-center gap-5 text-sm font-medium">
                ...
                <div class="relative" x-data="{ dropOpen: false }">
                    <button @click="dropOpen = !dropOpen" @click.away="dropOpen = false"
                        ...
                        Profil RS
                        <i class="fas fa-chevron-down text-xs transition-transform duration-200" :class="dropOpen ? 'rotate-180' : ''"></i>
                    </button>
                ...
                <div class="relative" x-data="{ layananOpen: false }">
                    <button @click="layananOpen = !layananOpen" @click.away="layananOpen = false"
                        ...
                        Layanan
                        <i class="fas fa-chevron-down text-xs transition-transform duration-200" :class="layananOpen ? 'rotate-180' : ''"></i>
                    </button>
                ...
                <div class="relative" x-data="{ infoOpen: false }">
                    <button @click="infoOpen = !infoOpen" @click.away="infoOpen = false"
                        ...
                        Informasi
                        <i class="fas fa-chevron-down text-xs transition-transform duration-200" :class="infoOpen ? 'rotate-180' : ''"></i>
                    </button>
                ...
                <a href="{{ route('home') }}#kontak" class="text-gray-700 hover:text-primary transition-colors">Kontak</a>
                <a href="{{ route('pendaftaran.form') }}" class="bg-gold hover:bg-gold-light text-white px-5 py-2 rounded-full font-semibold transition-colors shadow">
        ...
        <div x-show="open" x-transition class="md:hidden py-4 border-t border-gray-100">
            <div class="flex flex-col gap-1 text-sm font-medium">
                <a href="{{ route('home') }}#beranda" @click="open=false" class="text-gray-700 hover:text-primary py-2 px-2 rounded">Beranda</a>
                ...
                <div x-data="{ subOpen: false }">
                    ...
                    <div x-show="subOpen" class="pl-4 pb-1 space-y-1">
                        <a href="{{ route('profil.visi-misi') }}" @click="open=false" class="block text-gray-600 hover:text-primary py-1.5 px-2 rounded {{ request()->routeIs('profil.visi-misi') ? 'text-primary font-semibold' : '' }}">
                            <i class="fas fa-eye mr-2 text-primary text-xs"></i>Visi & Misi
                        </a>
                        <a href="{{ route('profil.profil-rs') }}" @click="open=false" class="block text-gray-600 hover:text-primary py-1.5 px-2 rounded {{ request()->routeIs('profil.profil-rs') ? 'text-primary font-semibold' : '' }}">
                            <i class="fas fa-hospital mr-2 text-primary text-xs"></i>Profil RS
                        </a>
                        <a href="{{ route('profil.direktur') }}" @click="open=false" class="block text-gray-600 hover:text-primary py-1.5 px-2 rounded {{ request()->routeIs('profil.direktur') ? 'text-primary font-semibold' : '' }}">
                ...
                <a href="{{ route('home') }}#kontak" @click="open=false" class="text-gray-700 hover:text-primary py-2 px-2 rounded">Kontak</a>
                <a href="{{ route('pendaftaran.form') }}" class="bg-gold text-white px-4 py-2 rounded-lg font-semibold text-center mt-2">
                  <i class="fas fa-clipboard-list mr-1"></i> Daftar Online
                </a>
            </div>
        </div>
