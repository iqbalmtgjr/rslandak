        ...
        <div class="flex items-center gap-3">
            @if (!empty($settings['facebook_url']))
                <a href="{{ $settings['facebook_url'] }}" target="_blank"
                    class="hover:text-gold-light transition-colors"><i class="fab fa-facebook-f"></i></a>
            @endif
            @if (!empty($settings['instagram_url']))
                <a href="{{ $settings['instagram_url'] }}" target="_blank"
                    class="hover:text-gold-light transition-colors"><i class="fab fa-instagram"></i></a>
            @endif
            @if (!empty($settings['youtube_url']))
                <a href="{{ $settings['youtube_url'] }}" target="_blank"
                    class="hover:text-gold-light transition-colors"><i class="fab fa-youtube"></i></a>
            @endif
        </div>
    </div>
</div>
